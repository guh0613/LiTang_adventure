...
class Pokemon:
    ...
    def _attack(self, pack: MsgPack):
        ...
        if pack.is_perfw():  # 结算穿透伤害
            ...
            if our.is_upgraded_attack:
                tmp = 1.2 * our.get_atk() - enemy.get_def()
            else:
    ...
    def init_skill(self, skill: str):
        ...
        if skill.startswith("丁真气焰"):
            self.logger.log(f"{self.name}的【丁真气焰】发动了！攻击力增加了{num}%")
            self.change_atk(skill, add_percent(num))
            return

        if skill.startswith("丁真气势"):
            self.logger.log(f"{self.name}的【丁真气势】发动了！攻击力增加了{num}点")
            self.change_atk(skill, add_num(num))
            return

        if skill.startswith("丁真强健"):
            self.logger.log(f"{self.name}的【丁真强健】发动了！防御力增加了{num}点")
            self.change_def(skill, add_num(num))
            return

        if skill.startswith("丁真强碱"):
            self.logger.log(f"{self.name}的【丁真强碱】发动了！防御力增加了{num}%")
            self.change_def(skill, add_percent(num))
            return

        if skill.startswith("运验丁真"):
            self.logger.log(f"{self.name}的【运验丁真】发动了！攻击力改为当前防御力的数值")
